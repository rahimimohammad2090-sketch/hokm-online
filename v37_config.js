...
function productionConfig(env=process.env){
  const secret=loadSecret({env,key:'AUTH_SECRET'});
  ...
  return {nodeEnv:env.NODE_ENV||'development',tlsEnabled:env.TLS_ENABLED==='true',persistenceDriver:String(env.PERSISTENCE_DRIVER||'memory').toLowerCase(),redisEnabled:env.REDIS_ENABLED==='true',authSecretLength:secret.length};
}
